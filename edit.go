...
func display(lines []DisplayLine) []string {
	...
	for i := range lines {
		...
		padding := max(1, maxLen-currentLen+1)
...
func editSettings(stateManager *StateManager) {
	...
	if statusReportData, ok := settingsData["status_report"].(map[string]any); ok {
		...
		if alerts, ok := statusReportData["alerts"].([]any); ok {
...
func addAlertsComments(data []byte) []byte {
	...
	rendered := display([]DisplayLine{
		...
		{4, "debug: false  # Enable verbose webhook logging", ""},
		{0, "", ""},
		{0, "my-email-alert:", ""},
		{2, "type: email", ""},
		{2, "enabled: true", ""},
		{2, "description: \"Email alerts example\"", ""},
		{2, "settings:", ""},
		{4, "smtp_host: smtp.gmail.com", ""},
		{4, "smtp_port: 587", ""},
		{4, "username: alerts@example.com", ""},
	...
	for name, alert := range alerts {
		...
		case "email":
			...
			if _, ok := alert.Settings["smtp_port"].(int); !ok {
				if _, okf := alert.Settings["smtp_port"].(float64); !okf {
					return fmt.Errorf("alert %s: email smtp_port is required", name)
				}
			}
			// password_env must be specified; no inline password support
